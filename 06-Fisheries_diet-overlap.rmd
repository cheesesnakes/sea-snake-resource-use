...
catch_fam%>%
  ...
  unnest()%>%
  dplyr::select(estimate1:p.value, d, -estimate)%>%
...
catch_fam%>%
  ...
  group_by(snake, Family)%>%
  summarise(N = length(unique(Family)), 
            p = mean(Rel.biomass, na.rm = T))%>%
  ggplot(aes(reorder(Family, p), p))+
  geom_col(width = 0.5)+
  scale_fill_brewer(palette = "Greys")+
  labs(x = "Fish Family", y = "Average proportion \n in catch")+
  facet_wrap(~snake, scales = "free_x")+
  theme(axis.text.x = element_text(angle = 60, hjust = 1, face = "italic"),
        strip.text = element_text(face = "italic"))

ggsave(last_plot(), filename = "./Figures and Tables/figure5.png", height = 4.5, width = 8)
```
