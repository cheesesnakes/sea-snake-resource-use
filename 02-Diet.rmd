...
gc%>%
  ...
  group_by(Snake.Species)%>%
  summarise(Prey.Species = length(unique(Prey.Species)), # removing unidentified
...
HC = as.data.frame(Diversity(fam_spade[,1])$Shannon_diversity)%>%
  rownames_to_column("Estimator")%>%
  mutate(Species = "Hydrophis curtus")

HS = as.data.frame(Diversity(fam_spade[,2])$Shannon_diversity)%>%
  rownames_to_column("Estimator")%>%
